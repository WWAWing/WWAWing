...
9. 環境変数 `GH_TOKEN` に GitHub のアクセストークンが入っている状態で,  ローカルのリポジトリの `all` パッケージのディレクトリに入り次を実行します。(GitHub のリリースに配布物がアップロードされます。) 

``` sh
$ WWA_WING_VERSION=vX.X.X npm run upload-zip # vX.X.X にはリリースに使ったタグ名を指定
```

アクセストークンについては, 下記ページを参照してください. (権限が必要です)

https://github.com/matsuyuki-a/wwawing-release-token
