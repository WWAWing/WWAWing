...
>[!NOTE]
WWA Wing Team のメンバーによって、状況によっては `master` ブランチ宛てのPull Requestに変更する場合があります。


## バージョニングポリシー
### 安定版
>[!TIP]
しばしば、正式版と表記されることもあります
...
### 不安定版 
不安定版のバージョンは、 `X.Y.Z-unstable.based-on.A.B.C` または  `X.Y.Z-unstable.based-on.A.B.C.p.n` という表記をしています。
...
- `p.n` の部分は、ベースにしている安定版リリースのバージョンが同じものが複数ある場合に、後から出たものに `n` の部分に大きい番号が与えられます。 `n` は 1 以上の整数値です。
  - ベースにしている安定版のバージョンが 1 つのみの場合はこの部分はありません。

例えば、 ベースにしている安定版バージョンが `5.5.0` 、リリース予定のバージョンが `5.6.0` の場合は次のようにバージョンが進んでいきます。

1. `5.6.0-unstable.based-on.5.5.0`
2. `5.6.0-unstable.based-on.5.5.0.p.1`: 不安定版の機能に不具合があって修正
3. `5.6.0-unstable.based-on.5.5.0.p.2`: 不安定版の機能にまた不具合があって修正
4. `5.6.0-unstable.based-on.5.5.1`: 安定版バージョン `5.5.1` がリリースされたので、その内容に追従した
5. `5.6.0-unstable.based-on.5.5.1.p.1`: 不安定版の機能にまたまた不具合があって修正
6. `5.6.0-unstable.based-on.5.5.2`: 安定版バージョン `5.5.2` がリリースされたので、その内容に追従した
...
## 急遽変更が必要になった (hotfix)
>[!NOTE]
>  (WWA Scriptリリースまでの時限措置) WWA Scriptに依存しない機能の安定版リリース今すぐをしたい場合も hotfix を実施してください。
