...
- **`develop`**: 最新の開発ブランチ (PR のデフォルトターゲット)
- **`master`**: 現在の安定版リリース
- **フィーチャーブランチ**: `develop` から作成し、`develop` にマージ

**コントリビューター向け**: リポジトリをフォークし、`develop` ブランチに向けて PR を作成してください。

**WWA Wing Team 向け**: リリースは `develop` を `master` にマージすることで行います (チームメンテナが実施)。
