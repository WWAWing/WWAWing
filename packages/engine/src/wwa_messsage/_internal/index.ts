  ...
  switch (currentLine.type) {
    ...
    case MacroType.SHOW_STR2:
      return shouldCreateParsedMessage
        ? new ParsedMessage(
            this._generateShowStrString(currentLine.macro.macroArgs, {
...
export function _generateShowStrString(
  macroArgs: string[],
  option: {
    triggerParts: TriggerParts;
    version: 1 | 2;
  ...
  return macroArgs.map((macroArg) => {
    ...
    if (!isNaN(parsedNumber)) {
      ...
      return () =>
        option.version === 1
          ? this._wwaData.userVar[parsedNumber]
          : parsedNumber;
    }
    ...
    if (parsedType) {
      return () =>
        ExpressionParser.evaluateMacroArgExpression(
          macroArg,
          this.generateTokenValues(option.triggerParts)
        );
    }
    // 引数を文字列として解釈する場合
    // \n は改行扱いされる。
