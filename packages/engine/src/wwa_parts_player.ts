...
export class Player extends PartsObject {
        ...
        if (id !== 0 && itemType !== ItemMode.NORMAL) {
            ...
            if (!this._isClickableItemGot) {
                if (mes !== "BLANK") {
                    ...
                    this._wwa.prepareSystemMessage(mes === "" ? deviceMessage : mes, false);
                }
        ...
        if (this._isPlayerTurn) {
            ...
            if (playerStatus.strength > enemyStatus.defence ||
                ...
                if (enemyStatus.energy > playerStatus.strength - enemyStatus.defence) {
                    ...
                    this._wwa.prepareAppearParts(this._enemy.position, AppearanceTriggerType.OBJECT, this._enemy.partsID);
                    this._state = PlayerState.CONTROLLABLE; // メッセージキューへのエンキュー前にやるのが大事!!(エンキューするとメッセージ待ちになる可能性がある）
                    this._wwa.prepareMessage(this._enemy.message, false, this._enemy.position);
    ...
    public readyToUseItem(itemPos: number): void {
        ...
        this._wwa.prepareAppearParts(this._position.getPartsCoord(), AppearanceTriggerType.OBJECT, itemID);
